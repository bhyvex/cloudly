...
def _get_memory_usage():
    ...
    if platform.system() == 'Darwin':
        ...
        ps = subprocess.Popen(['top','-l 1', '-s 0'], stdout=subprocess.PIPE).communicate()[0]

        for line in ps.split('\n'):
            if('PhysMem' in line):

                memory_used = int(line.split(' ')[1].replace('M',''))*1024*1024/1000
                memory_free = int(line.split(' ')[-2].replace('M',''))*1024*1024/1000
